...
from src.grammar_preprocessor import GrammarPreprocessor
from src.models.prop_pred import PropertiesPredictionModel
from src.utils import LoadSmilesData
...
parser.add_argument(
    "--evaluation",
    help="Evaluate the model on prior validity, novelty and uniqueness",
    action="store_true",
)
parser.add_argument(
    "--train_property",
...
parser.add_argument(
    "--hyper_optim",
    help="Perform hyperparameter optimization over the property prediction model",
    action="store_true",
)
parser.add_argument(
    "--reconstruction",
    help="Estimates the reconstruction accuracy",
    action="store_true",
)
parser.add_argument(
    "--active_units", help="Calculates the number of active units", action="store_true"
)
...
data = LoadSmilesData(hyper_params["labels_path"], normalization="none")
...
model = GrammarPreprocessor(hyper_params)
...
def normalize_data():
    ...
    if hyper_params["normalization"].lower().strip() == "minmax":
        minmaxscaler = MinMaxScaler()
        property_train_normalized = minmaxscaler.fit_transform(
            np.asarray(property_train).reshape(-1, 1)
        )
        scaler = minmaxscaler

    elif hyper_params["normalization"].lower().strip() == "standard":
        standardscaler = StandardScaler()
        property_train_normalized = standardscaler.fit_transform(
            np.asarray(property_train).reshape(-1, 1)
        )
        scaler = standardscaler

    elif hyper_params["normalization"].lower().strip() == "none":
...
def plot_latent_space(t_sne=True):
    ...
    pca_data = pca.fit_transform(z[: hyper_params["n_data_plot"]])
    plt.scatter(
        pca_data[:, 0],
        pca_data[:, 1],
        c=property_train[: hyper_params["n_data_plot"]],
        cmap="viridis",
        s=5,
    )
    cbar = plt.colorbar()
    ...
    plt.title(hyper_params["plot_title"], fontsize=18)
    plt.xlabel("PCA 1", fontsize=17)
    plt.ylabel("PCA 2", fontsize=17)
    plt.locator_params(nbins=5)
    plt.tight_layout()
    plt.savefig(
        os.path.join(evaluation_path, f"{hyper_params['prop']}_latent_space.png"),
        dpi=300,
    )
    plt.show()
    ...
    if t_sne:
        ...
        tsne_data = tsne.fit_transform(z[: hyper_params["n_data_plot"]])
        plt.scatter(
            tsne_data[:, 0],
            tsne_data[:, 1],
        )
        ...
        plt.savefig(
            os.path.join(
                evaluation_path, f"{hyper_params['prop']}_latent_space_tsne.png"
            ),
            dpi=300,
        )
        plt.show()
...
def model_evaluation():
    ...
    with open(hyper_params["labels_path"], "rb") as data:
        df = pickle.load(data)
    ...
    valid_smiles = [
        Chem.MolToSmiles(x, isomericSmiles=True, canonical=True) for x in mols
    ]
    valid_smiles[:] = [
        x for x in valid_smiles if x
    ]  # Will remove None values from valid_smiles
...
def active_units(delta=0.01):
    ...
    testloader = DataLoader(
        smiles_test, batch_size=hyper_params["batch"], drop_last=True, shuffle=False
    )
...
def property_model_training():
    ...
    train_split, validation_plit = random_split(
        train_data, [len(train_data) - chunk, chunk]
    )

    trainloader = DataLoader(
        train_split, batch_size=hyper_params["batch"], drop_last=True, shuffle=False
    )
    validloader = DataLoader(
        validation_plit, batch_size=hyper_params["batch"], drop_last=True, shuffle=False
    )

    # loading the model
    pp_model = PropertiesPredictionModel(
        hyper_params["latent_dim"], hyper_params["hidden_layer_prop_pred"]
    )
    ...
    for i in range(5):
        ...
        for epoch in range(epochs):
            ...
            for x, label in trainloader:
                ...
                loss = criterion(
                    predictions.view(-1).to(device), label.float().to(device)
                )
            ...
            with torch.no_grad():
                for x_val, label_val in validloader:
                    ...
                    loss_val = criterion(
                        predictions_val.view(-1).to(device),
                        label_val.float().to(device),
                    )
        ...
        if np.mean(valid_loss) < min_valid_loss:
            ...
            print(
                f"Validation loss decreased from {min_valid_loss:.6f} to {np.mean(valid_loss):>6f}. Saving the model!"
            )
            torch.save(
                pp_model.state_dict(),
                os.path.join(evaluation_path, "prop_pred_model.pth"),
            )
    ...
    print(
        f"The MSE of the hold-out set is: {np.mean(mse):.5f} \u00b1 {np.std(mse):.5f}"
    )
    print(
        f"The RMSE of the hold-out set is: {np.mean(rmse):.5f} \u00b1 {np.std(rmse):.5f}"
    )
...
def hyperparameter_optimization(fraction=0.5):
    ...
    class PropertiesPredictionModelHyperparameterOptimization(torch.nn.Module):
        """
        ...
        def __init__(self, input_dim=56, hidden_units=512, dropout=0.5):
            super(PropertiesPredictionModelHyperparameterOptimization, self).__init__()
    ...
    pp_model = PropertiesPredictionModelHyperparameterOptimization()

    property_train_normalized, _ = normalize_data()
    x, y = (
        z[: int(len(z) * fraction)],
        property_train_normalized[: int(len(z) * fraction)],
    )
    ...
    hyparameters = {
        "lr": [0.001, 0.0001],
        "max_epochs": [30, 50],
        "module__input_dim": [hyper_params["latent_dim"]],
