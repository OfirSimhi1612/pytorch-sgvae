...
def plot_latent_space(t_sne=True):
    ...
    im = plt.scatter(pca_data[:, 0], pca_data[:, 1], c=property_train[:hyper_params['n_data_plot']], cmap='viridis', s=5)
    cbar = plt.colorbar()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(evaluation_path, f"{hyper_params['prop']}_latent_space.png"), dpi=300)
    plt.show()
    
    if t_sne:
        tsne = TSNE(n_components=2)
        tsne_data = tsne.fit_transform(z[:hyper_params['n_data_plot']])
        im = plt.scatter(tsne_data[:, 0], tsne_data[:, 1], c=property_train[:hyper_params['n_data_plot']], cmap='viridis', s=5)
        cbar = plt.colorbar()
        tick_locator = ticker.MaxNLocator(nbins=5)
        cbar.locator = tick_locator
        cbar.update_ticks()
        ...
        plt.locator_params(nbins=5)
        plt.tight_layout()
        plt.savefig(os.path.join(evaluation_path, f"{hyper_params['prop']}_latent_space_tsne.png"), dpi=300)
        plt.show()
...
def active_units(delta=0.01):
    ...
    """
    testloader = DataLoader(smiles_test, batch_size=hyper_params['batch'], drop_last=True, shuffle=False)
...
def property_model_training():
    ...
    validloader = DataLoader(validation_plit, batch_size=hyper_params['batch'], drop_last=True, shuffle=False)
    
    # loading the model
    pp_model = PropertiesPredictionModel(hyper_params['latent_dim'], hyper_params['hidden_layer_prop_pred'])
    
    if torch.cuda.is_available():
        pp_model.cuda()
    
    optimizer = torch.optim.Adam(pp_model.parameters(), lr=hyper_params['learning_rate_prop'], amsgrad=True)
