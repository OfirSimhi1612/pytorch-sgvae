...
def prop_run():
    saving_path = (
        ...
        + f"_{today.hour}_{today.minute}_{today.second}"
    )

    epochs = hyper_params["epochs"]
    batch = hyper_params["batch"]
    input_dim = hyper_params[
        "input_dim"
    ...
    # loading the data
    dataset = TrainQM9dataset(
        hyper_params["dataset_path"],
        hyper_params["labels_path"],
        hyper_params["normalization"],
    )[:4000]
    ...
    optimizer = torch.optim.Adam(
        list(model.parameters()) + list(pp_model.parameters()),
    ...
    if hyper_params["anneal_kl"]:
        anneal = AnnealKL(
            n_epoch=epochs,
            n_cycle=hyper_params["n_cycle"],
            ratio=hyper_params["ratio_anneal_kl"],
        )
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (x, label) in batch_pbar:
            ...
            reconstruction_loss = input_dim * criterion(
                x_decoded_mean.view(-1), x.view(-1)
            )
            kl = model.kl(mu, sigma)
            property_loss = pp_loss(predictions.view(-1), label.to(device).float())
            if hyper_params["anneal_kl"]:
                elbo = (
                    recons_weight * reconstruction_loss
                    + kl_weight * kl * beta
                    + property_loss * prop_weight
                )
            else:
                elbo = (
                    recons_weight * reconstruction_loss
            ...
            batch_pbar.set_postfix(
                {"Batch Loss": f"{elbo.item():.4f}", "ETA": f"{remaining/60:.1f} min"}
            )
        # saving the results
        if hyper_params["anneal_kl"]:
        ...
        with torch.no_grad():
            for batch_idx, (x_val, label_val) in enumerate(validloader):
                x_val = (
                    x_val.transpose(1, 2).contiguous().to(device)
                )  # [batch, 76, 100]
                ...
                reconstruction_loss_val = input_dim * criterion(
                    x_decoded_mean_val.view(-1), x_val.view(-1)
                )
                kl_val = model.kl(mu_val, sigma_val)

                property_loss_val = pp_loss(
                    predictions_val.view(-1), label_val.to(device).float()
                )

                if hyper_params["anneal_kl"]:
                    elbo_val = (
                        recons_weight * reconstruction_loss_val
                        + kl_weight * kl_val * beta
                        + property_loss_val * prop_weight
                    )
                else:
                    elbo_val = (
                        recons_weight * reconstruction_loss_val
        ...
        log_val["reconstruction"].append(
            (avg_recons_val * recons_weight) / len(validloader)
        )
        ...
        if min_valid_loss > avg_elbo_val:
            ...
            torch.save(
                ...
                os.path.join(saving_path, "gvae_encoder.pth"),
            )
            # decoder
            torch.save(
                model.decoder.state_dict(),
                os.path.join(saving_path, "gvae_decoder.pth"),
            )
...
def no_prop_run():
    ...
    for epoch in range(epochs):
        ...
        for x, _ in trainloader:
            ...
            # training procedure -----------------------------------------------------
            x = (
                x.transpose(1, 2).contiguous().to(device)
            )  # [batch, NUM_OF_RULES, MAX_LEN]
            z, mu, sigma, logits = model(x)
        ...
        print(
            f"ELBO: {(avg_elbo/len(trainloader)):>5f} \tKL: {(avg_kl/len(trainloader)):>5f} \tReconstruction loss: {(avg_recons/len(trainloader)):>5f}"
        )
        ...
        with torch.no_grad():
            ...
            for x_val, _ in validloader:
                ...
                if hyper_params["anneal_kl"]:
                    elbo_val = (
                        hyper_params["reconstruction_weight"] * reconstruction_loss_val
                        + hyper_params["kl_weigth"] * kl_val * beta
                    )
                else:
                    elbo_val = (
                        hyper_params["reconstruction_weight"] * reconstruction_loss_val
                        + hyper_params["kl_weigth"] * kl_val
                    )
            ...
            print(
                f"ELBO_val: {(avg_elbo_val/len(validloader)):>5f} \tKL_val: {(avg_kl_val/len(validloader)):>5f} \tReconstruction_val: {(avg_recons_val/len(validloader)):>5f}"
            )

            if min_valid_loss > avg_elbo_val:
                print(
                    f"Validation loss decreased ({(min_valid_loss/len(validloader)):.5f} ---> {(avg_elbo_val/len(validloader)):.5f}) \t Saving the model"
                )
                min_valid_loss = avg_elbo_val

                torch.save(
                    model.state_dict(), os.path.join(saving_path, "gvae_model.pth")
                )
                # encoder
                torch.save(
                    model.encoder.state_dict(),
