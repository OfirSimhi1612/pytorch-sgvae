...
class Decoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.linear_in = nn.Linear(
            hyper_params["latent_dim"], hyper_params["latent_dim"]
        )
        self.linear_out = nn.Linear(501, num_in_channels)

        self.rnn = nn.GRU(
            input_size=hyper_params["latent_dim"],
            hidden_size=501,
    ...
    def forward(self, z):
        ...
        h = h.unsqueeze(1).expand(
            -1, hyper_params["input_dim"], -1
        )  # [batch, MAX_LENGHT, latent_dim] This does the same as the repeatvector on keras
        h, _ = self.rnn(h)
