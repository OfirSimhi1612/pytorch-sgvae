...
class LoadSmilesData:
    ...
    def __init__(self, labels_path, normalization):
        ...
        if normalization.lower().strip() == 'minmax':
            ...
            self._property_train_normalized_div = minmaxscaler.fit_transform(property_train_div)
            self._property_test_normalized_div = minmaxscaler.transform(property_test_div)
            
        elif normalization.lower().strip() == 'standard':
            standardscaler = StandardScaler()
            self._property_train_normalized = standardscaler.fit_transform(property_train)
            self._property_test_normalized = standardscaler.transform(property_test)
            self._property_train_normalized_div = standardscaler.fit_transform(property_train_div)
            self._property_test_normalized_div = standardscaler.transform(property_test_div)
